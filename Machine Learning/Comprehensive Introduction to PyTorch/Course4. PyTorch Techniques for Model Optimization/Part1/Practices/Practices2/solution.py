...
# Initialize checkpointing parameters
best_loss = float('inf')
checkpoint_path = "best_model.pth"

# Training loop with model checkpointing based on training loss
num_epochs = 100
for epoch in range(num_epochs):
    model.train()
    optimizer.zero_grad()
    ...
    if loss.item() < best_loss:
        best_loss = loss.item()
        torch.save(model, checkpoint_path)
        print(f"Model saved at epoch {epoch} with training loss {loss.item():.4f}!")

    # Validate the model (not used for saving in this version)
    model.eval()
    with torch.no_grad():
        val_outputs = model(X_valid)
        val_loss = criterion(val_outputs, y_valid).item()
