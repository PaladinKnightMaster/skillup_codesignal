...
const fetchBooksByDate = async (publishedDate: string) => {
  const query = `
    query {
      books(publishedDate: "${publishedDate}") {
        title
        author
        publishedDate
        genre
  ...
  try {
    ...
    const data = await response.json();
    console.log(`Books Published in ${publishedDate}:`, JSON.stringify(data, null, 2));
  } catch (error) {
    console.error('Error:', error);
  }
};

fetchBooksByDate('1937');
