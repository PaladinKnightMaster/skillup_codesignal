...
// Changing password
const changePassword = async (username: string, newPassword: string, token: string): Promise<any | null> => {
  const query = `
  ...
  `;
  const variables = { username, newPassword };
  const response = await executeQuery(query, variables, token);
...
(async () => {
  ...
  const updatedUser = await changePassword('user', 'newpassword', token);
